...
$objeto=json_decode($datos);//pasar json a objeto php


if($objeto != null) {
  switch($objeto->servicio) { //sacamos atributos del objeto php
    case "listar":
    	print json_encode(listadoPersonas()); //json_encode convierte el php a json
      break;
    case "insertar":
...
function modificarPersona($objeto) {
	global $conn;
	try {
		$sql = "UPDATE personas SET DNI	= ?, NOMBRE	= ?, APELLIDOS	= ? WHERE id = ?";
		$conn->prepare($sql)->execute(
		array(
			$objeto->dni,
			$objeto->nombre, 
